...
var deleteTask = function(taskId) {
    ...
    for (var i = 0; i < tasks.length; i++) {
        //if tasks[i].id doesn't match the value of taskId, let's keep that task and push ot into the new array
        if (tasks[i].id !== parseInt(taskId)) {
...
var taskStatusChangeHandler = function(event) {
    ...
    //find tge parent task item element based on the id
    var taskSelected = document.querySelector(".task-item[data-task-id='" + taskId + "']");
...
var loadTasks = function() {
    ...
    // convert task from stringify back into an array
    savedTasks = JSON.parse(savedTasks);
    for (var i = 0; i < savedTasks.length; i++) {
        createTaskEl(savedTasks[i]);
    }

    // creates tasks elements back onto the page
}
